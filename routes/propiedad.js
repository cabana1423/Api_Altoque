...
router.delete("/",/*midleware,*/ async(req, res) => {
 ...
 router.put("/",/*midleware,*/ async(req, res) => {
    ...
    var allowkeylist = ["nombre","nit","propietario","telefono","hubicacion.calle"];
 ...
 router.put("/put-img",/*midleware,*/ async(req, res) => {
    ...
    PROP.update({_id:  params.id}, {$set: {"img_prop":[{"titulo":sing+ "_" +img.name,"pathfile":totalpath}]}}, (err, docs) => {
        if (err) {
            res.status(500).json({msn: "Existen problemas en la base de datos"});
             return;
         } 
